...
| 2026-09-16 | (this commit — see `git blame` for the hash) | WP-E6-SWEEP 66→9 band-aggregation rule (`C₉ = W C₆₆ Wᵀ`, `P₉ = W P₆₆`) | None — pinned amendment §8 resolution item 2 delegated the scheme to WP-E6-BINMAP without fixing it; BINMAP-C returned `aggregated_9x9: null` rather than invent one | Inverse-variance weights on the member diagonal (`w_{b,i} ∝ 1/σᵢ²`, normalised per band), full 66×66 covariance propagated — `briefs/WP_E6_SWEEP_DESIGN_PINNED_2026_09_16.md` §1.2 | T0 ruling A1 2026-09-16 (`briefs/T0_RULINGS_2026_09_16.md`; Option 1 of `T0_DECISION_REQUEST_SWEEP_AGGREGATION_2026_07_31.md`). Post-pin analysis choice per amendment §5 ⇒ logged; every comparison consuming `C₉`/`P₉` is `exclusion`/`FIT` (already foreclosed from `TEST`, now mechanically so). |
