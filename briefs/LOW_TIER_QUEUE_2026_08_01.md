...
| A-S3-4 | SWEEP still blocked on T0's 66→9 aggregation ruling (`7ca1846`) — not a T2 task, listed here only so this queue doesn't imply it's actionable. | — | **RULED 2026-09-16: Option 1 approved** (see ruling appended to `briefs/T0_DECISION_REQUEST_SWEEP_AGGREGATION_2026_07_31.md`). Aggregation blocker cleared; implementation (pinned design doc + `binmap.py` step + tests) owed, not started. |
