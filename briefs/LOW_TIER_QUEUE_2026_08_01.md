...
| A-S3-4 | SWEEP still blocked on T0's 66→9 aggregation ruling (`7ca1846`) — not a T2 task, listed here only so this queue doesn't imply it's actionable. | — | **RULED 2026-09-16: Option 1 approved** (see ruling appended to `briefs/T0_DECISION_REQUEST_SWEEP_AGGREGATION_2026_07_31.md`). Aggregation blocker cleared; implemented 2026-09-16 (pin `79c9444`, code/artifact `a3ffcfd`, verification brief). Gate now closed only on design §4 (k_target vs k_eff) + independent re-run. |
...
| B-S2-3 | LR-2 (enumerate classified sporadics into a CSV, ≥15 sequences with OEIS IDs/weights/modularity) — pure tabulation from already-cited literature (Zagier 2009, Cooper 2012, AESZ). | T2 | **DONE 2026-08-01, continuation part 3** — reconsidered the earlier deferral once it became clear this could be built from already-hash-pinned in-repo citations (`refs/recurrences_v1.json`, 11 entries) rather than 15 fresh web lookups, avoiding the exact rushed-verification risk that motivated deferring it. `scripts/render_lr2_classified_sporadics.py` (S2 sandbox branch, `fd91c8d`), mechanically generated, no field typed by hand. **While building this, found and flagged a real, live cross-repo contradiction**: t103 is DROPPED in S1's `K3_CRITERIA.md` (2026-07-18) but S2's own later `ESCALATIONS.md` E-014 (2026-07-26) found the drop was based on a conflation with a different candidate and no T0 veto ever existed — see A-S2-1's update above and S1 `briefs/T0_FLAG_K3_CRITERIA_T103_STALE_2026_08_01.md`. Deliberately excluded t103/S22 from the CSV pending that T0 ruling — chose 11 honestly-sourced entries over padding to 15. |
...
automatically since it changes system-level session behavior, flagged for Xavier.) **Correction 2026-09-16:** this item did NOT close the durability gap —
with `Linger=no` the unit stopped with each login session; the harvest log shows runs only on
08-01/02, 08-23, 08-27/28, 09-10, 09-12, 09-15/16. T0 directed linger on 2026-09-16; enabled
that day (`loginctl show-user` → `Linger=yes`).
