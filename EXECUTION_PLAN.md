...
| **Second use** | WP-H series, `docs/WP_H_AUTO_RESEARCH_TRIAGE.md` (T0-authorized per `docs/WP_H_T0_AUTHORIZATION_2026_07_25.md`) |
...
- **v1.3 (2026-07-25):** WP-H triaged an externally-supplied auto-research brief (`briefs/SOURCE_autoresearch_brief_2026_07_25.md`, vendored verbatim with its SHA256) into a merge-blocking hypothesis registry (`pipeline/hypothesis_registry.py`): 6 of 26 claims runnable, 17 blocked on absent data/provenance, 4 out of scope. Executed the runnable subset on the T4 (`scripts/wp_h_auto_research.py`, `docs/WP_H_AUTO_RESEARCH_TRIAGE.md`), second use of the `SANDBOX-EXPERIMENTAL` label. Findings feed Stream 2's M1 memo as input only; the brief's τ = 0.0000 + 1.21145i and τ_imag = 0.972 are recorded as fabricated constants and regression-tested against reimport.
